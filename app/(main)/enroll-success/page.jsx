...
const Success = async ({ searchParams: { session_id, courseId } }) => {
  if (!session_id) {
    throw new Error("Please provide a valid session id that starts is cs_");
  }
  ...
  const paymentInten = checkoutSession?.payment_intent;
  const paymentStatus = paymentInten?.status;
  ...
  if (paymentStatus === "succeeded") {
    ...
    const emailSendResponse = await sendEmails(eamilToSend)
