...
const EnrolledCourseCard = async ({enrollment}) => {
    const courseCategory = await getCategoriesDetails(enrollment?.course?.category?._id)
    ...
    const totalCorrect = quizzesTaken.map((quiz)=>{
        ...
        return item.filter((res)=>{
            return res.isCorrect === true && res.isSelected === true
        })
				...
				<div className="flex flex-col pt-2">
					...
					<div className=" border-b pb-2 mb-2">
						<div className="flex items-center justify-between">
							...
							<p className="text-md md:text-sm font-medium text-slate-700">
								Completed Modules <Badge variant="success">{totalCompletedModule}</Badge>
							</p>
						</div>
