...
const EditCourse = async ({params:{courseId}}) => {
  ...
  const mappedCategory = categories.map((item)=>{
    return {
 ...
 const modules = replaceMongoIdArray(course?.modules).sort((a,b)=>a.order - b.order)
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16">
          <div>
            ...
            <CategoryForm initialData={{label:course?.category?.title}} courseId={courseId}  options={mappedCategory}/>

            <QuizSetForm initialData={{quizSetId: course?.quizSet?._id.toString()}} courseId={courseId} options={mappedQuizSets} />
          </div>
